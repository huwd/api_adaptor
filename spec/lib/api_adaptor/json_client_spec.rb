...
RSpec.describe ApiAdaptor::JsonClient do
  ...
  describe "client" do
    it "can use basic auth" do
      ...
      stub_request(:put, "http://some.endpoint/some.json").with(:basic_auth => (["user", "password"])).to_return(:body => "{\"a\":1}", :status => 200)
    ...
    it "govuk headers ignored in requests if not present" do
      ...
      ApiAdaptor::JsonClient.new.get_json("http://some.other.endpoint/some.json")
      assert_requested(:get, /\/some.json/) do |request|
        (not request.headers.key?("X-Govuk-Authenticated-User"))
      end
    end
  ...
  describe "#get_json" do
    describe "client" do
      it "can set custom headers on gets" do
        stub_request(:get, "http://some.other.endpoint/some.json").to_return(:status => 200)
        ApiAdaptor::JsonClient.new.get_json("http://some.other.endpoint/some.json", "HEADER-A" => "B", "HEADER-C" => "D")
        assert_requested(:get, /\/some.json/) do |request|
  ...
  describe "#post_json" do
    describe "client" do
      it "can set custom headers on gets" do
        ...
        ApiAdaptor::JsonClient.new.post_json("http://some.other.endpoint/some.json", {}, "HEADER-A" => "B", "HEADER-C" => "D")
  ...
  describe "#put_json" do
    describe "client" do
      it "can set custom headers on puts" do
        ...
        ApiAdaptor::JsonClient.new.put_json("http://some.other.endpoint/some.json", {}, "HEADER-A" => "B", "HEADER-C" => "D")
    ...
    describe "responses" do
      it "are always considered present and not blank" do
        ...
        expect((not response.blank?)).to(be_truthy)
  ...
  describe "#delete_json" do
    describe "client" do
      it "can set custom headers on deletes" do
        ...
        ApiAdaptor::JsonClient.new.delete_json("http://some.other.endpoint/some.json", {}, "HEADER-A" => "B", "HEADER-C" => "D")
  ...
  describe "#post_multipart" do
    it "can post multipart responses" do
      url = "http://some.endpoint/some.json"
      stub_request(:post, url).with(:headers => ({ "Content-Type" => /multipart\/form-data; boundary=----RubyFormBoundary\w+/ })) do |request|
  ...
  describe "#put_multipart" do
    it "can post multipart responses" do
      url = "http://some.endpoint/some.json"
      stub_request(:put, url).with(:headers => ({ "Content-Type" => /multipart\/form-data; boundary=----RubyFormBoundary\w+/ })) do |request|
        request.body =~ /------RubyFormBoundary\w+\r\nContent-Disposition: form-data; name="a"\r\n\r\n123\r\n------RubyFormBoundary\w+--\r\n/
      end.to_return(:body => "{\"b\": \"1\"}", :status => 200)
